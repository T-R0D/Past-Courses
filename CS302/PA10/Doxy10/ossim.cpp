...
int main()
{
  ...
  for( minute = 0 ; minute < simLength ; minute++ )
  {
    ...
    if( ( 1 <= numArrivals ) && ( numArrivals <= 2 ) )
    {
      ...
      {
        /*
          WAS UNABLE TO GET APPROPRIATE ANSWER FROM INSTRUCTOR ABOUT THIS. IN
          PRACTICE, THIS IS APPROXIMATELY HOW I WOULD USE A PRIORITY QUEUE AND
          I WOULD REPORT THAT THE ITEM COULD NOT BE ADDED, BUT I DID NOT REPORT
          BECAUSE I WAS AFRAID OF HAVING DIFFERING CODE FROM THE TEST CASES.
        */

        // case: the queue is not full
        if( !taskPQ.isFull() )
        {
          // add a new task
