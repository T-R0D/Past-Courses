...
DataType List<DataType>::getCursor() const throw (logic_error) {
  ...
  // case: list is empty
    ...
    throw logic_error("Data is not convertible to int data type");
  }
}
...
////////////////////////////////////////////////////////////////////////////////
template <typename DataType>
void List<DataType>::BubbleSortL2H() {
  // variables
  int num_swaps = 1;
  DataType data_holder;

  // if the list has members, sort it
  if( !isEmpty() ) {
    // only bother if there is more than one node in the list
    if( head->next != NULL ) {
      // as long as swaps are made with a pass, keep sorting
      while( num_swaps > 0 ) {
        // start at beginning of list, iterate across
        for( gotoBeginning(), num_swaps = 0; cursor->next != NULL; 
             cursor = cursor->next ) {
          // if values are in wrong order, swap them
          if( cursor->dataItem > cursor->next->dataItem ) {
            // make the swap
            data_holder = cursor->next->dataItem;
            cursor->next->dataItem = cursor->dataItem;
            cursor->dataItem = data_holder;
