...
float getMatrixElement( Matrix* matrix, unsigned int i, unsigned int j );
...
Matrix* createMatrix( unsigned int height, unsigned int width )
{
  ...
  new_matrix->data = (float*) calloc( matrix_size, sizeof( float ) );
...
void matrixRandomFill( Matrix* matrix )
{
  ...
  for( i = 0; i < matrix->i_size; i++ )
  {
    ...
    for( j = 0; j < matrix->j_size; j++ )
    {
      ...
      setMatrixElement( matrix, i, j, (float) ( rand() % 5 ) );
    }
  }
