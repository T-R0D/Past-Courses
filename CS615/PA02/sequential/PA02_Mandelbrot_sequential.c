...
*/
  ...
  if( program_status == kSuccess )
  {
    ...
    // compute all the rows to produce the Mandelbrot image
    for( row_ndx = 0; row_ndx < mandel_image->height; row_ndx++ )
    {
      // compute the current Mandelbrot image row
      computeMandelbrotImageRow( mandel_image->data[row_ndx], row_ndx,
                                 mandel_image->width, mandel_image->height );
    }

    // end the timed period
    computation_time = stopwatch( 'x' );
