/**
    @file PA02_Mandelbrot_dynamic.cpp

    @author Terence Henriod

    Parallel Mandelbrot Computation: Dynamic Load Distribution

    @brief This program produces an image representative of the
           canonical Mandelbrot Set using dynamic work allocation. The time
           required for the computations is also recorded for comparison
           with parallel algorithms that conduct the same operation. 

    @version Original Code 1.00 (3/4/2014) - T. Henriod
...
double dynamicMandelbrotCollect( int num_processors,
                                 PGMImageData* mandel_image );

void dynamicMandelbrotCompute( int processor_rank, int num_processors,
                               PGMImageData* mandel_image );
...
int main( int argc, char** argv )
{
  ...
  if( program_status == kSuccess )
  {
    ...
    if( processor_rank == kMaster )
    {
      ...
      computation_time = dynamicMandelbrotCollect( num_processors,
                                                   mandel_image );
    ...
    else
    {
      ...
      dynamicMandelbrotCompute( processor_rank, num_processors, mandel_image );
    }
                                 ...
                                 PGMImageData* mandel_image )
{
  ...
  int slave_id = 0;
  int slave_number = 0;
  int num_slaves = num_processors - 1;
  int next_row_to_do = 0;
  int block_start_row = 0;
  int row_being_received = 0;
  int last_row_in_block = 0;
  int num_rows_received = 0;
  double elapsed_time = 0;
  MPI_Status message_status;

  // compute the starting row of the first block to be sent
  next_row_to_do = num_slaves * kBlockOfRows;
...
// TODO: This works fine when the image size is a multiple of kBlockOfRows,
//       but fix for other cases to be more adaptable

  // collect all the rows
  while( num_rows_received < mandel_image->height )
  {
    // receive a row number that indicates a block of completed rows 
    MPI_Recv( &block_start_row, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
              MPI_COMM_WORLD, &message_status );

    // store that slave's id
    slave_id = message_status.MPI_SOURCE;

    // receive all data rows that slave has completed
    for( row_being_received = block_start_row,
         last_row_in_block = block_start_row + kBlockOfRows;
         row_being_received < last_row_in_block;
         row_being_received++ )
    {
      // receive the row
      MPI_Recv( (mandel_image->data)[row_being_received], mandel_image->width,
                MPI_CHAR, slave_id, MPI_ANY_TAG, MPI_COMM_WORLD,
                &message_status );
    }

    // count the rows as received
    num_rows_received += kBlockOfRows;

    // send the starting row number of the next block to be processed
    MPI_Send( &next_row_to_do, 1, MPI_INT, slave_id, kMandelTag,
              MPI_COMM_WORLD );

    // move to the next row number to be sent
    next_row_to_do += kBlockOfRows;
  }

  // send a signale to the remaining processes to stop computation
  for( slave_number = 1; slave_number < num_processors; slave_number++ )
  {
    // case: this slave is not the one who got assigned the last block
    if( slave_number == slave_id )
    {
      // send the stop signal (the next row to do will now be too large)
      MPI_Send( &next_row_to_do, 1, MPI_INT, slave_id, kMandelTag,
                MPI_COMM_WORLD );
    }
  }
                               ...
                               PGMImageData* mandel_image )
{
  ...
  int end_marker = 0;
  int current_row = 0;
  char** mandel_buffers = NULL;
  MPI_Status message_status;

  // allocate the computation buffer
  mandel_buffers = allocateImageMatrix( mandel_image->width,
                                        kBlockOfRows );

  // compute the starting row number
  start_row = ( processor_rank - 1 ) * kBlockOfRows;
  ...
  while( start_row < mandel_image->height )
  {
    // compute the limiting row value
    end_marker = start_row + kBlockOfRows;

    // compute all the rows in the block
    for( current_row = start_row; current_row < end_marker; current_row++ )
    {
      // compute the row, store result in the appropriate buffer
      computeMandelbrotImageRow( mandel_buffers[current_row - start_row],
                                 current_row, mandel_image->width,
                                 mandel_image->height );
    }

    // send the starting row number back
    MPI_Send( &start_row, 1, MPI_INT, kMaster, kMandelTag, MPI_COMM_WORLD );

    // send the row data back
    for( current_row = start_row; current_row < end_marker; current_row++ )
    {
      // send the row
      MPI_Send( mandel_buffers[current_row - start_row], mandel_image->width,
                MPI_CHAR, kMaster, kMandelTag, MPI_COMM_WORLD );
    }

    // receive a starting row number
    MPI_Recv( &start_row, 1, MPI_INT, kMaster, MPI_ANY_TAG, MPI_COMM_WORLD,
              &message_status );
  }

  // the master will stop the stopwatch at this point

  // deallocate the mandel_buffer
  deallocateImageMatrix( mandel_buffers, kBlockOfRows );
