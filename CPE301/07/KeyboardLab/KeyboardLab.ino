...
const unsigned long int DEBOUNCE = 20;       // in milliseconds
const unsigned long int U_DEBOUNCE = 2000;   // in microseconds

static byte byteRead;  // necessary global variable
...
unsigned char pollForInput( volatile unsigned char previousInput );

void driveSpeaker( unsigned char input );
...
{
  // variables
  static volatile unsigned char input;
  
  // given code needed to make the lab work
  ...
  // drive the speaker for 0.25 seconds (this will acually be run indefinitely with no new input
  // if a change is not detected after the 0.25 seconds)
  driveSpeaker( byteRead );
...
unsigned char pollForInput( volatile unsigned char previousInput )
{
  ...
  volatile unsigned char input = previousInput;
  
  // check the port to see if the input differed
  if( *portB != previousInput )
  {
    // wait for debouncing
    MyDelay( DEBOUNCE );
    
    // check to see if a new input is still detected
    if( *portB != previousInput )
    {
  ...
  // return whatever input was detected
  return input;
}
...
{
  // variables
  unsigned long int frequency = 440; // in Hz
  
  // set proper frequency to ouput
  switch( input )
  {
    // sound A
    case 0x00:
...
{
  // variables
  volatile unsigned char* portB = (unsigned char*) 0x25;  // for B
  ...
  // I want the wave to run for 0.25 seconds, find out how many wave periods per second
  counter = (unsigned long int) (250000 / period); // x1000000 to adjust for useconds
