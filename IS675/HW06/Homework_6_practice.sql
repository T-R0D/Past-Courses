...
*/
select JobTask.TaskID,
	convert(decimal,sum(JobTask.EstHours))/convert(decimal,sum(JobTask.SquareFeet)) as EstimatedHoursPerSqft
from JobTask
where JobTask.DateCompleted is not null
group by JobTask.TaskID;

create view [Task_Hours_BreakDown] as
select Task.TaskID,
	   Task.TaskDescription,
	   convert(Decimal,isNULL(sum(JobTask.EstHours),0.000000))
				/
	   convert(Decimal, sum(JobTask.SquareFeet)) as EstimatedHoursPerSqFt
from Task left join JobTask on Task.TaskID = JobTask.TaskID 
group by Task.TaskID, Task.TaskDescription;

--select * from [Task_Hours_BreakDown] as Task2 order by Task2.TaskID, Task2.TaskDescription;
--drop view [Task_Hours_Breakdown] --view that combines the est hours based on taskID (does aggregate est per square feet)
--select * from JobTask order by JobTask.TaskID;

create view [JobtaskTimeSheet] as
select ah.TaskID, 
		convert(decimal,isNULL(sum(ah.ActualHoursWorked),0.000000))
		/
		convert(decimal, sum(jobTask.SquareFeet)) as ActualHours 
from [actual_hours] as ah
join JobTask on ah.TaskID = JobTask.Taskid 
group by ah.TaskID;
-- drop view [JobTaskTimeSheet];

select 
		[Task_Hours_BreakDown].TaskID,
		[Task_Hours_BreakDown].TaskDescription,
		[Task_Hours_BreakDown].EstimatedHoursPerSqFT,
		(case
			when [JobTask].[DateCompleted] is NULL
				then NULL
				else [JobtaskTimeSheet].ActualHours
			end) as ActualHours,
		(case
			when [Task_Hours_BreakDown].EstimatedHoursPerSqFT is NULL
				then 'Null Estimate'
			when [JobTaskTimeSheet].ActualHours is NULL
				then 'Null Actual'
			when [Task_Hours_BreakDown].EstimatedHoursPerSqFT > [JobTaskTimeSheet].ActualHours
				then 'Estimate Larger'
			else  'Actual Larger'
		end)	as ComparisonMessage
	from [Task_Hours_BreakDown] left join [JobTaskTimeSheet]
	on [Task_Hours_BreakDown].TaskID = [JobtaskTimeSheet].TaskID,
	JobTask
	where [Task_Hours_BreakDown].TaskID = JobTask.TaskID
order by  [Task_Hours_BreakDown].TaskID;

select
	JTTS.T	
 from [Task_Hours_BreakDown] as THB
 left join [JobTaskTimeSheet] as JTTS
 on THB.TaskID = JTTS.TaskID;

-- drop view [JobTask_TimeSheet_Hours_per_sqft];
