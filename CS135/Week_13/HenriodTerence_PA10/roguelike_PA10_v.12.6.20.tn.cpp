...
/*
  ...
  Developer: Nicholas Smith
*/
void readHiScores ( char names[NUM_SCORES][ NAME_LEN ], int scores[] );
...
Developer: Terence Henriod
*/
void clearMatrix( short matrix[PLAY_AR_H][PLAY_AR_W] );
...
Developer: Terence Henriod
*/
void setFloor( int floorNumber, short playMatrix[PLAY_AR_H][PLAY_AR_W] );
...
Developer: Terence Henriod
*/
void genObjectAt( int Xcoord, int Ycoord, short object, short playMatrix[PLAY_AR_H][PLAY_AR_W] );
...
Developer: Terence Henriod
*/
void matrixToScreen(int matrixX, int matrixY, short playMatrix[PLAY_AR_H][PLAY_AR_W] );
...
/*
  ...
  Developer: Nicholas Smith
*/
void movePlayer ( int waitTime, bool &continueProgram, int &intendedFloor, int &cloaks, 
                  int &swords, int &spells, int &treasure, short gameBoard[PLAY_AR_H][PLAY_AR_W], bool xcMode,int &score  );
...
bool collisionTest( int &cloakTally,  int &swordTally,  int &spellTally, int &treasureTally, int &intendedFloor,
                    int intendedX, int intendedY, short playMatrix[PLAY_AR_H][PLAY_AR_W], bool &continueGame );
...
Developer: Terence Henriod
*/
void screenToMatrix( int screenX, int screenY, short object, short playMatrix[PLAY_AR_H][PLAY_AR_W] );
...
Developer: Terence Henriod
*/
void genObjects( bool xcMode, short gameBoard[ PLAY_AR_H ][ PLAY_AR_W ] );
...
/*
  ...
  Developer: Nicholas Smith
*/
void XYPos ( int &xPos, int &yPos, short playMatrix[PLAY_AR_H][PLAY_AR_W] );
...
/*
  ...
  Developer: Nicholas Smith
*/
bool spaceOccupied ( int XPos, int YPos, short playMatrix[PLAY_AR_H][PLAY_AR_W]);
...
/*
  ...
  Developer: Nicholas Smith
*/
void writeScoreFile( char names[NUM_SCORES][ NAME_LEN ], int scores[NUM_SCORES], int candidate, char playerName[NAME_LEN] );
...
/*
  ...
  Developer: Nicholas Smith
*/
void sortScores( char names[NUM_SCORES][ NAME_LEN], int scores[NUM_SCORES], int candidate, char playerName[]);
...
/*
  ...
  Developer: Nicholas Smith
*/
void showHiScores ( char names[NUM_SCORES][NAME_LEN], int scores[NUM_SCORES] );
       ...
       for( index = 0; index < NUM_SCORES; index ++ )
         {
          ...
          for(lindex = 0; lindex < NAME_LEN; lindex ++)
             {
             names[index][lindex] = SPACE;
             }

          names[index][lindex-1] = NULL_CHAR;
         }
  ...
  while( keepRunning ) 
     ...
     // display menu
        // clear screen
           ...
           clearScreen( MENU_UPX-3, MENU_UPY-1, (MENU_UPX + MENU_W+4), (MENU_UPY + MENU_H-1) );         
          ...
          switch( userInput )
             ...
             case 'p':
                ...
                // write high score file
                  ...
                  writeScoreFile( names, scores, score, playerName );
                }
                break;
...
char dispSplash ()
    ...
    // Display Developer's Names
       ...
       printStringAt (Xpos, Ypos, "Implemented by: Terence Henriod & Nicholas Smith", "LEFT");
...
void dispInstructions( bool xMode )
   ...
   // paint gray background
      ...
      clearScreen( INSTR_UPX+1, INSTR_UPY+2, (INSTR_UPX + INSTR_W+1), (INSTR_UPY + INSTR_H+2) );
      ...
      // paint a bigger background if extra credit mode is enabled
         ...
         if( xMode )
           {
           //function: clearScreen
            clearScreen( INSTR_UPX+1, (INSTR_UPY + INSTR_H), (INSTR_UPX + INSTR_W+1), (INSTR_UPY + INSTR_H + 5) );
           }
...
void readHiScores ( char names[NUM_SCORES][ NAME_LEN ], int scores[NUM_SCORES] )
   {
    // Initalize Variables
    int index = 0;
    int namIndex = 0;
    ...
    // Loop to read in files, go until index < 11 || fin !.good
       ...
       fin >> tempScore;     // prime loop

       while (index < NUM_SCORES && fin.good() )
        { 
            scores[index] = tempScore;

         // catch the comma delimeter
            fin >> commaDumpster;

         // read in name into array
            fin.getline(names[namIndex], NAME_LEN);
...
char printMenuText()
   ...
   // prompt for user input
      ...
      yLoc += 2;
      printStringAt( xLoc, yLoc, "Enter Selection: ", "LEFT" );
...
void promptForPlayer( char name[ NAME_LEN ] )
   ...
   // prompt for user name, storing the name in a C-style string
      // functiproon: mptForStringAt (the void one)
      promptForStringAt( xPos, yPos, "Enter Your Name: ");
...
int executeGame( bool xcMode, int &score, int difficulty )
   ...
   // variables
      ...
      // game board matrix
      short gameBoard[PLAY_AR_H][PLAY_AR_W];
   ...
   // begin game loop
      ...
      while( keepPlaying )
        ...
        // manage player movement 
           // function: movePlayer
           movePlayer( difficulty, keepPlaying, goToFloor, cloaks, swords, spells, treasure, gameBoard, xcMode, score );
...
void clearMatrix( short matrix[PLAY_AR_H][PLAY_AR_W] )
   ...
   // pick a row and fill it, then move on
      // operation: for
      for( rIndex = 0; rIndex < PLAY_AR_H; rIndex ++ )
         {
         // iterate through the row
            // operation: for
            for( cIndex = 0; cIndex < PLAY_AR_W; cIndex ++ )
               {
               // store a space to the current matrix position
               matrix[rIndex][cIndex] = SPACE;
               // loop ends when every position in row is filled
               }

         // loop ends when all rows have been visited 
         }
...
void setFloor( int floorNumber, short playMatrix[PLAY_AR_H][PLAY_AR_W] )
   ...
   // load walls into game matrix by looping through coordinates until the walls have been "used up"
      ...
      for( index = 0; index < numWalls; index ++ )
         {
         xPosition   =   xCoords[index] ;
         yPosition   =   yCoords[index] ;
         playMatrix[yPosition][xPosition]=IN_WALL;
         }
   ...
   // reload stairs, depending on level
      ...
      switch( floorNumber )
         ...
         case 1:
               ...
               for(index = 0; index < numWalls; index ++)
                  {
                  genObjectAt( yCoords[index], xCoords[index], IN_WALL, playMatrix );
                  }
               break;
...
int readFloorData( const char fileName[], int Xcoordinates[], int Ycoordinates[] )
   ...
   // use loop to read the floor pattern data into the coordinate arrays
      // operation: for
       while(fin.good()) 
         {
            Xcoordinates[index] = xPosition ; 
            Ycoordinates[index] = yPosition ;  
            index++; 
            // Re prime
            fin>>xPosition;
            fin>>yPosition; 
...
void genObjectAt( int Xcoord, int Ycoord, short object, short playMatrix[PLAY_AR_H][PLAY_AR_W] )
   ...
   // animate object on screen
      ...
      matrixToScreen( Xcoord, Ycoord, playMatrix );
...
void movePlayer ( int waitTime, bool &continueProgram, int &intendedFloor, int &cloaks, 
    ...
    // Initialize variables
       ...
       xLoc = GAMESTART_X + SCOREBOARD_W + 2;
       yLoc = GAMESTART_Y + 1;
       xVector = 0; yVector = 0;
    do
       {
        ...
        // decide if player can move into next spot
           // operation: if
              if ( collisionTest( cloaks, swords, spells, treasure, intendedFloor, 
                   (xLoc + xVector), (yLoc + yVector), gameBoard, continueProgram ) )
                 {
...
bool move( int &xPos, int &yPos, int xVect, int yVect )
   {
    ...
    if( ( xVect != 0 ) // is meant to move
          && ( xPos + xVect >= SCOREBOARD_W + 2 ) // won't go off left side of screen 
    ...
    // if y vector is live, test for acceptable movement limits
               ...
               && ( yPos + yVect >= SCRN_MIN_Y + 1 ) // won't go off top of screen
         ...
         if( xcMode )
           ...
           // sword generation
              ...
              if( doGenObject( P_SWORD ) )
                ...
                setColor( COLOR_GREEN, COLOR_WHITE, SET_BRIGHT );
                XYPos( xPos, yPos, gameBoard );
                genObjectAt( xPos, yPos, SWORD, gameBoard );
                }
           ...
           // monster generation
              ...
              if( doGenObject( P_MONSTER ) )
                ...
                setColor( COLOR_RED, COLOR_WHITE, SET_BRIGHT );
                XYPos( xPos, yPos, gameBoard );
                genObjectAt( xPos, yPos, MONSTER, gameBoard );
                }
           ...
           // spell generation
              ...
              if( doGenObject( P_SPELL ) )
                ...
                setColor( COLOR_GREEN, COLOR_WHITE, SET_BRIGHT );
                XYPos( xPos, yPos, gameBoard );
                genObjectAt( xPos, yPos, SPELL, gameBoard );
                }
           }
...
void writeScoreFile( char names[NUM_SCORES][ NAME_LEN ], int scores[NUM_SCORES], int candidate, char playerName[NAME_LEN] )
   {
    ...
    // Sort scores from highest to lowest
       // sortScores
       sortScores( names, scores, candidate, playerName);

    // write the scores, score then name
       // fout
...
void sortScores( char names[NUM_SCORES][ NAME_LEN], int scores[], int candidate, char playerName[] )
   {
    // Initilize Variables
    ...
    for ( index = NUM_SCORES - 1; index > 0 && !inserted; index-- )
    {
        if ( candidate > scores[index] )
           {
            // move player down
            if ( index+1 < NUM_SCORES )
               {
           ...
           {
            // place under current then break
            if ( index+1 < NUM_SCORES )
               {
                strcpy(names[index+1], playerName);
                scores[index+1] = candidate;
               }
            inserted = true;
           }
    }
...
void showHiScores ( char names[NUM_SCORES][NAME_LEN], int scores[NUM_SCORES] )
   {
    ...
    for( index = 0; index < NUM_SCORES; topY ++, index ++ )
       {

       printStringAt( topNameX, topY, names[index], "LEFT");

       printIntAt( topScoreX, topY, scores[index], "RIGHT" );
       }
