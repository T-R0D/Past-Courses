...
cout << endl << "Message size: " << messageSize << endl;;



   // encrypt message
      // function: encodeData
      encodeData( message, encodedMessage, 100, codeSize );
...
int readData( char inName[], char message[], bool &fileThere, bool &fileTooLarge, int &messSize )
   ...
   // if file is of acceptable size, set the flag to indicate such
      // operation: if
      if( index < ( MAX_MESSAGE - 1 ) )     // indicates that there is room for the null terminator
        {
         // set flag
         fileTooLarge = false;
...
void oversizeError( char inName[] )
   ...
   // display the message
      // function: cout
      cout << "ERROR: The message found in " << inName << " is too large."
...
void encodeData( char message[], int encodedMessage[], int messSize, int &codeSize )
   ...
   // load the code array, using a loop, with both true and false values, to end of message
      // operation for
      for( messIndex = 0, codeIndex = 0; messIndex < messSize; codeIndex ++ )
        {
        // decide whether to load an even, true value, or an odd dummy one
           // operation: if
           if( doLoadEven( EVEN_PROB ) )
             {
             // convert the character value into a coded even
                // cast the character value as an integer
                tempVal = int ( message[ messIndex ] );

                // complete the encoding process
                   // function: encodeVal
                   tempVal = encodeVal( tempVal, EVEN );
   ...
   codeSize = codeIndex;     // codeIndex should refer to the first invalid position 
...
void writeEncodedMessage( char outName[], int encodedMessage[], int codeSize )
   ...
   // write data to file using 8 character c-strings
      // operation: for
      for( outputCount = 0; outputCount < codeSize; outputCount ++)
        {
        // only write 5 values per line
           // operation: for
           for( valOnLine = 0; valOnLine < MAX_VAL_PER_LINE; valOnLine ++ )
             {
...
void stringConversion( char tempString[], int codeVal )
   ...
   // use loop to input the coded value in reverse order
      // operation: for
      for( strngIndex = ( OUT_STRNG_LEN - 2 ); tempInt > 0; strngIndex -- )
        {
        ...
        tempString[ strngIndex ] = tempChar;
